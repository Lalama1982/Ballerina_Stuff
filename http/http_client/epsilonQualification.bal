...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string hsdCode = "hsdCode", 
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Training Qualification HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Training Qualification HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_qualification
    json|error respJson = check dtlClient->/postInsertTrainingQualification.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Training Qualification", respJson);
        return;
    }    
