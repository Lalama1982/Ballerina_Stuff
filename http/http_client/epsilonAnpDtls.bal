...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string hsdEpsilonidentifier = "hsdEpsilonidentifier", 
                     string hsdLegalname = "hsdLegalname", 
                     string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Training ANP Details HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Training ANP Details HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_anp_dtls
    json|error respJson = check dtlClient->/postInsertTrainingAnpDtls.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Training ANP Details", respJson);
        return;
    }    
