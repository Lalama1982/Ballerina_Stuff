...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string address1Line1 = "address1Line1", 
                     string address1Line2 = "address1Line2", string address1City = "address1City", string address1Postalcode = "address1Postalcode",
                     string address1State = "address1State", string address1Country = "address1Country", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Employer Physical Address HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Employer Physical Address HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_empl_physical_addr
    json|error respJson = check dtlClient->/postInsertEmplPhysicalAddr.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Employer Physical Address", respJson);
        return;
    }    
