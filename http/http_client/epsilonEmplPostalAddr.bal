...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string address2Line1 = "address2Line1", 
                     string address2Line2 = "address2Line2", string address2City = "address2City", string address2Postalcode = "address2Postalcode",
                     string address2State = "address2State", string address2Country = "address2Country", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Employer Postal Address HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Employer Postal Address HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_empl_postal_addr
    json|error respJson = check dtlClient->/postInsertEmplPostalAddr.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Employer Postal Address", respJson);
        return;
    }    
