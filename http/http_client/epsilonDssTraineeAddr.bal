...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string address2Line1 = "address2Line1", 
                     ...
                     string hsdAddress2state = "hsdAddress2state", string hsdAddress2country = "hsdAddress2country", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Trainee Address HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Trainee Address HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_trainee_addr
    json|error respJson = check dtlClient->/postInsertTraineeAddr.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Trainee Addresses", respJson);
        return;
    }    
