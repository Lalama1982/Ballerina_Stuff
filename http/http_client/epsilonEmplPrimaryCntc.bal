...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string telePhone1 = "telePhone1", 
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Employer Primary Contact HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Employer Primary Contact HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_empl_primary_cntc
    json|error respJson = check dtlClient->/postInsertEmplPrimaryCntc.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Employer Primary Contact", respJson);
        return;
    }    
