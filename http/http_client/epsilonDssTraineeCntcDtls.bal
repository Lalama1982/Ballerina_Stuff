...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string telePhone2 = "telePhone2,", string mobilePhone = "mobilePhone",
        string emailAddress1 = "emailAddress1", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS CNTC Trainee Details HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Trainee CNTC Details HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_traineecntc_dtls
    json|error respJson = check dtlClient->/postInsertTraineeCntcDtls.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS CNTC Trainee Details", respJson);
        return;
    }    
