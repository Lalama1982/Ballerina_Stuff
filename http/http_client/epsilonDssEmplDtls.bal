...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string hsdEpsilonidentifier = "hsdEpsilonidentifier", 
                     ...
                     string hsdAbn = "hsdAbn", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Employer Details HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Employer Details HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_empl_dtls
    json|error respJson = check dtlClient->/postInsertEmplDtls.post({    
        "_postInsertEmplDtls": {
            "hdrSeq": hdrSeq,
            "dtlSer": dtlSer,
            "recSts": hdrFlag,
    ...
    if respJson is error {
        io:println("Failed to merge DSS Employer Details", respJson);
        return;
    }    
