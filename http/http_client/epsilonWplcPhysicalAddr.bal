...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string address1Line1 = "address1Line1", 
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS WPLC Physical Address HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS WPLC Physical Address HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_wplc_physical_addr
    json|error respJson = check dtlClient->/postInsertWplcPhysicalAddr.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS WPLC Physical Address", respJson);
        return;
    }    
