...
public function main(int hdrSeq = 0, int dtlSer = 2, int hdrFlag = 1, string hsdPrevioustrainingcontract = "hsdPrevioustrainingcontract", string recMsg = "recMsg") returns error? {
    ...
    if dtlClient is error {
        io:println("Failed to initialize DSS Trainee Pre-VTC Details HTTP client", dtlClient);
        return;
    }
    io:println(string `Initialized DSS Trainee Pre-VTC Details HTTP client with URL: ${urlDss}, timeout: ${timeout}, HTTP version: ${httpVersion}`);
    ...
    // tbl_epsilon_traineeprevtc_dtls
    json|error respJson = check dtlClient->/postInsertTraineePrevtcDtls.post({    
    ...
    if respJson is error {
        io:println("Failed to merge DSS Trainee Pre-VTC Details", respJson);
        return;
    }    
