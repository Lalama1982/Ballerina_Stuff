...
public function getRecrContacts(http:Client dssClient,
        string? examId = "", string rResp = "rResp") returns Root|error {

    Root|error respJson = check dssClient->get(string `/getRecrContacts?examId=${examId ?: ""}`,
        headers = {
    ...
    if respJson is error {
        log:printInfo("getRecrContacts :: Failed to fetch Client Contact Details", respJson);
        return error("getRecrContacts :: Failed to fetch Client Contact Details", respJson);
    }

    //log:printInfo(`getRecrContacts :: respJson : ${respJson}`);
    return respJson;
}

# Description : Update assessments results to table "hit_reports.br2r_bal_client_contacts"
#
# + dssClient - HTTP Client for DSS EP  
